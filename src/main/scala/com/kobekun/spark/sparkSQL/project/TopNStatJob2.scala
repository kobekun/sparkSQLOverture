...
//    accessDF.show(false)
    ...
    //按照流量进行统计
    trafficAccessTopNStat(spark,commonDF)
  ...
  def trafficAccessTopNStat(spark: SparkSession, commonDF: DataFrame) = {

    val trafficAccessTopNDS = commonDF.groupBy("day","cmsId")
...
//    videoAccessTopNDF.show(false)
  ...
  def cityAccessTopNStat(spark: SparkSession, commonDF: DataFrame) = {
    ...
    /**
      * 按照dataframe的方式进行统计
      */
    val cityAccessTopNDS = commonDF.groupBy("day","city","cmsId")
      .agg(count("cmsId").as("times"))
